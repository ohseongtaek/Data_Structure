...
int main (int argc, char *argv[])
{
   ...
   for (i=0;i<num_ref;i++)
    ...
    /* main loop */
    in_index=0;
    for (i=0;i<num_in*vec_dim/4;i+=vec_dim) {
    
       /* distance calculation */
	   ref_index=0;
       for (j=0;j<num_ref*vec_dim/4;j+=vec_dim) {
       
          	dist=_mm_setzero_ps();
	  		for (k=0;k<vec_dim/4;k++) {
				temp = _mm_sub_ps(ref_SSE[(j+k)*4], in_SSE[(i+k)*4]);
				temp = _mm_mul_ps(temp, temp);
				dist = _mm_add_ps(dist, temp);
	  		}
          	result[ref_index].dist=my_dist[dist];
